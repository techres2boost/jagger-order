  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const { data: livreur, error: livreurError } = await supabase
        ...
        .eq("user_id", u.user.id)
        .maybeSingle();
      console.log("[livreur] fiche livreurs (via user_id):", livreur, "| error:", livreurError);
      if (!mounted || !livreur) {
        console.warn(
          "[livreur] aucune fiche livreurs liée à ce user_id (ou accès RLS refusé) → arrêt, liste vide",
        );
        setLoading(false);
        return;
      }
    ...
    async function loadOrders(id: string) {
      console.log(
        "[livreur] requête orders — filtre assigned_livreur_id =",
        id,
      ...
      console.log("[livreur] nombre de commandes 'delivering' assignées:", list.length);
      if (!mounted) return;
      setOrders(list);
