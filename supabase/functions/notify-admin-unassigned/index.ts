...
// Edge Function invoquée par le worker `process_delivery_assignments` (via pg_net)
// quand une commande prête ne trouve AUCUN livreur disponible et passe en file
// d'attente (pending_assignment = true). Envoie un push aux abonnements admin
// (push_subscriptions.role = 'admin'). Flux isolé : ne touche ni au client ni au
// livreur. Une alerte visible dans le dashboard admin sert de fallback fiable.
...
serve(async (req: Request) => {
  try {
    ...
    console.error("notify-admin-unassigned error", e);
